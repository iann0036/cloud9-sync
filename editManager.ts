...
class PendingEdit {
    private action_list:string[];

    constructor() {
        this.action_list = [];
    }

    getActionListItemLength(i:number) {
    ...
    splitActionListItem(i:number, cutpoint:number) {
        if (this.action_list[i][0] == "r") {
...
export class EditManager {
    private pending_edit;
    private last_unacknowledged_edit;
    public lastKnownRemoteDocumentText;
    public lastKnownLocalDocumentText;
    public revNum;
    public vfsid;
    public recent_edits;
    private recent_edits_iterator;
    private doc_path;
    ...
    sendPendingEdits() {
        ...
        setTimeout(() => {
            ...
            let seq = this.websocketProvider.send_ch4_message(
                ["call","collab","send",[this.vfsid,{"type":"EDIT_UPDATE","data":{"docId":this.doc_path,"op":edit,"revNum":this.revNum,"selection":[
    ...
    addRemoteEdit(edit: vscode.WorkspaceEdit) {
    ...
    processTextDocumentChange(change, evt) {
        ...
        this.queuePendingEdit(
            (change.rangeOffset != 0 ? change.rangeOffset.toString() : null),
            (change.rangeLength > 0 ? delText : null),
            (change.text.length > 0 ? change.text : null)
        );
